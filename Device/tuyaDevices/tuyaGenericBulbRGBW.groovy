...
metadata {
	definition(name: "Local Tuya - 2", namespace: "iholand", author: "iholand") {
		capability "Actuator"
		capability "Bulb"
...
preferences {
	section("tuya Device Config") {
		...
		input name: "logDecrypt", type: "bool", title: "Enable decrypt logging", defaultValue: false, description: "<small>If issues are experienced it might help to turn on decrypt logging and see the decrypt logs, automatically turned off after 30 min. Check device IP, ID and local key make sure they are correct. Also a power off/on on the tuya device might help.</small>"
...
def setIpError(status, message="") {
    ...
    if (status && !currentStatus.contains("true")) {
        ...
        sendEvent(name: "ipError", value : value)
        sendEvent(name: "ipErrorDatetime", value : 0)
    }
...
boolean socket_connect() {
	...
	try {
		...
		log.error "$ex - Can't connect to device, make sure correct IP address, try running 'python -m tinytuya scan' to verify, also try to power device on and off"
		setIpError(true, "Socket Timeout at ipAddress")
        returnStatus = false
	} catch (e) {
		log.error "Error $e"
		returnStatus = false
...
def sendTimeout() {
	if (state.retry > 0) {
        ...
        setIpError(true, "No answer from device after 5 retries")
		socket_close()
	}
}
...
Map decodeIncomingFrame(byte[] incomingData, Integer sofIndex=0, byte[] testKey=null, Closure callback=null) {
	...
	if (status != null && status != [:]) {
		sendEvent(name: "_rawMessage", value: status.dps)
	}
